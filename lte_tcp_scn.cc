...
int main (int argc, char *argv[])
{
    ...
    for (uint32_t i = 0; i < remotesHostContainer.GetN (); i++){
        ...
        if(i==0 && i !=remotesHostContainer.GetN()-1) {
            ...
            BulkSendApplicationTcpTrace (serverApps.Get(serverApps.GetN ()-1)->GetObject<BulkSendApplication> (), traceFilePrefixLte, Seconds (ltesourcestart + 0.2), Seconds (simulationEnd));
            ReceiverTcpTrace (ueContainer.Get(i)->GetObject<Ipv4L3Protocol> (), traceFilePrefixLte, Seconds (simulationEnd));
        }
